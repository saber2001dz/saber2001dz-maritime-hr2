...
interface UseCongesEmployeesTrendReturn {
...
const fetcher = async (isRTL: boolean): Promise<TrendData> => {
  ...
  // Récupérer le nombre actuel d'employés en congé
  ...
  if (currentError) {
    console.error("Error fetching current conges employees:", currentError)
    throw currentError
  }

  // Récupérer le nombre d'employés en congé du mois précédent
  ...
  if (previousError) {
    console.error("Error fetching previous month conges employees:", previousError)
    // En cas d'erreur, on ne peut pas calculer la tendance
  }

  // Debug logging
  console.log('🔍 Conges Trend Debug:', {
    today: todayStr,
  ...
  console.log('📊 Conges Trend Result:', trendData)

  return trendData
}

export function useCongesEmployeesTrend(
  isRTL: boolean
): UseCongesEmployeesTrendReturn {
  const { data, error, isLoading } = useSWR<TrendData>(
    ["conges-employees-trend", isRTL],
    () => fetcher(isRTL),
    {
