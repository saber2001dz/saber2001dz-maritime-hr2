...
interface UseAbsentEmployeesTrendReturn {
...
const fetcher = async (isRTL: boolean): Promise<TrendData> => {
  ...
  // Récupérer le nombre actuel d'employés absents
  ...
  if (currentError) {
    console.error("Error fetching current absent employees:", currentError)
    throw currentError
  }

  // Récupérer le nombre d'employés absents du mois précédent
  ...
  if (previousError) {
    console.error("Error fetching previous month absent employees:", previousError)
    // En cas d'erreur, on ne peut pas calculer la tendance
  }

  // Debug logging
  console.log('🔍 Absent Trend Debug:', {
    today: todayStr,
  ...
  console.log('📊 Absent Trend Result:', trendData)

  return trendData
}

export function useAbsentEmployeesTrend(
  isRTL: boolean
): UseAbsentEmployeesTrendReturn {
  const { data, error, isLoading } = useSWR<TrendData>(
    ["absent-employees-trend", isRTL],
    () => fetcher(isRTL),
    {
