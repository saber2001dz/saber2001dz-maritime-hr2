...
interface SuspendedEmployeesPopoverProps {
  triggerClassName?: string
}

export function SuspendedEmployeesPopover({ triggerClassName = "" }: SuspendedEmployeesPopoverProps) {
  ...
  const { suspendedEmployees, isLoading, error, totalSuspended } = useSuspendedEmployees()
    ...
    if (suspendedEmployees.length === 0) {
      return (
        <div className="text-sm text-muted-foreground py-4 text-center">
          {isRTL ? "لا يوجد موظفون موقوفون عن العمل حالياً" : "Aucun employé suspendu actuellement"}
        </div>
      )
    }
  ...
  return (
    <Popover>
      ...
      <PopoverContent
        className="w-200 bg-white dark:bg-[#1C1C1C]"
